...
export async function getAlsoSearched(keyword: string) {
  ...
  $('div[data-template-id="itemKeywordGroup"][data-template-type="alsoSearch"] a[data-template-id="itemKeyword"]').each((i, elem) => {
    const title = $(elem).find('span.sds-comps-text.sds-comps-text-ellipsis-1').text().trim();
...
export async function getSmartBlocks(keyword: string) {
  ...
  try {
    ...
    console.log(`>>> 최종 추출된 스마트블록 키워드 개수: ${smartBlocks.length}`);
...
export async function getPopularPosts(keyword: string) {
  ...
  try {
    ...
    const popularPosts: { title: string; link: string; content: string; author: string; date: string }[] = [];
    ...
    if (popularContainer.length === 0) {
      ...
      console.log(`>>> 전체 페이지에서 data-meta-area 속성을 가진 요소 ${allMetaAreas.length}개 발견:`);
    ...
    if (popularElements.length === 0) {
      const alternativeElements = $('div[data-meta-area="ugB_qpR"] .fds-article-simple-box');
      console.log(`>>> 대안 선택자로 인기글 요소 ${alternativeElements.length}개를 찾았습니다.`);
    ...
    popularElements.each((i, elem) => {
      ...
      if (i === 0) {
        ...
        titleSelectors.forEach((selector, idx) => {
          ...
          elements.each((j, el) => {
            console.log(`>>>     - 텍스트: "${$(el).text().trim()}", 링크: "${$(el).attr('href') || '없음'}"`);
          });
        });
        ...
        $(elem).find('a').each((j, el) => {
          ...
          console.log(`>>>   ${j + 1}. 텍스트: "${text}", 링크: "${href}", 클래스: "${classes}"`);
        });
      ...
      console.log(`>>> 인기글 ${i + 1}: 제목="${title}", 작성자="${author}", 날짜="${date}", 내용="${content.substring(0, 50)}..."`);

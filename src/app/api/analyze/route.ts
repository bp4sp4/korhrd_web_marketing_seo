...
export async function POST(request: NextRequest) {
  try {
    ...
    const smartBlockKeywords = smartBlocks.filter(
      (keyword) => keyword.length > 0
    );

    // 연관 검색어를 문자열 배열로 변환
    const alsoSearchedKeywords = alsoSearched
    ...
    const detailedAnalysis = {
      ...
      contentFreshness: calculateFreshnessScore(validatedPopularPosts),
      competitionLevel: calculateCompetitionLevel(analysis.totalResults),
      seoInsights: generateSEOInsights(
        validatedPopularPosts,
        smartBlockKeywords,
        alsoSearchedKeywords,
        keyword
      ...
      searchVolume: calculateSearchVolume(
        validatedPopularPosts,
        smartBlockKeywords,
        alsoSearchedKeywords
      ),
...
function analyzeResults(
  ...
  // 키워드 추출 및 분석 (이미 문자열 배열이므로 그대로 사용)
  const smartBlockKeywords = smartBlocks.filter(
    (keyword) => keyword.length > 0
  );

  const alsoSearchedKeywords = alsoSearched.filter(
    (keyword) => keyword.length > 0
  );
  ...
  const totalResults =
    popularPosts.length +
    smartBlockKeywords.length +
    alsoSearchedKeywords.length;
  ...
  // C-RANK 점수 계산
  const cRankScore = calculateCRankScore(
    popularPosts,
    smartBlocks,
    alsoSearched
  );
  ...
  // 상위 노출 확률 계산
  const exposureProbability = calculateExposureProbability(
    cRankScore,
    totalResults
  );
...
function extractTopKeywords(text: string, mainKeyword: string): string[] {
  ...
  morphemes.forEach((morpheme) => {
    if (
      ...
      keywordScores[morpheme.word] =
        (keywordScores[morpheme.word] || 0) + score;
    }
  });
...
function performMorphemeAnalysis(
  text: string
  ...
  koreanWords.forEach((word) => {
    ...
    // 부사 패턴
      ...
      word.match(
        /.*(게|히|이|로|에|에서|부터|까지|도|만|은|는|이|가|을|를|의|와|과|하고|며|고|지만|라도|라도)$/
      )
    ) {
      pos = "MAG";
    }
...
function calculateCRankScore(
  ...
  if (popularPosts.length > 0) {
    popularPosts.forEach((post, index) => {
      ...
      const contentScore =
        contentLength >= 100 && contentLength <= 300
          ? 10
          : contentLength >= 50 && contentLength <= 500
          ? 7
          : contentLength >= 20 && contentLength <= 800
          ? 5
          : 3;
      ...
      const keywordScore =
        keywordDensity >= 0.02 && keywordDensity <= 0.05
          ? 10
          : keywordDensity >= 0.01 && keywordDensity <= 0.08
          ? 7
          : keywordDensity >= 0.005 && keywordDensity <= 0.1
          ? 5
          : 3;

      // 각 인기글의 종합 노출 점수
      score +=
        (rankScore + titleScore + contentScore + keywordScore) *
        (1 - index * 0.1); // 순위가 낮을수록 가중치 감소
    });
  } else {
...
function calculateFreshnessScore(posts: PopularPost[]): number {
  let score = 0;
...
function generateSEOInsights(
  ...
  if (popularPosts.length === 0) {
    insights.push(
      "이 키워드는 아직 인기글이 없습니다. 새로운 콘텐츠로 시장을 선점할 기회입니다."
    );
    insights.push(
      "최신 정보와 실용적인 내용으로 차별화된 콘텐츠를 작성해보세요."
    );
    insights.push("인기글이 없으면 검색 결과 상위 노출이 어려울 수 있습니다.");
  } else {
    // 제목 길이 분석
    const avgTitleLength =
      popularPosts.reduce((sum, post) => sum + post.title.length, 0) /
      popularPosts.length;
    if (avgTitleLength < 20) {
      insights.push(
        "제목이 너무 짧습니다. 20-40자로 늘려보세요. (검색 노출 최적화)"
      );
    } else if (avgTitleLength > 50) {
      insights.push(
        "제목이 너무 깁니다. 20-40자로 줄여보세요. (검색 노출 최적화)"
      );
    }

    // 키워드 밀도 분석
    const allText = popularPosts
      .map((post) => post.title + " " + post.content)
      .join(" ");
    const keywordDensity = calculateKeywordDensity(allText, keyword);
    if (keywordDensity < 0.01) {
      insights.push(
        "키워드 밀도가 낮습니다. 자연스럽게 키워드를 더 포함해보세요. (노출 점수 향상)"
      );
    } else if (keywordDensity > 0.05) {
      insights.push(
        "키워드 밀도가 너무 높습니다. 과도한 키워드 삽입을 피하세요. (스팸 방지)"
      );
    }

    // 콘텐츠 신선도 분석
    const recentPosts = popularPosts.filter(
      (post) =>
        post.date.includes("시간 전") ||
        (post.date.includes("일 전") &&
          parseInt(post.date.match(/(\d+)일/)?.[1] || "0") <= 7)
    );
    if (recentPosts.length < popularPosts.length * 0.3) {
      insights.push(
        "최신 콘텐츠가 부족합니다. 최근 1주일 내 콘텐츠를 더 작성해보세요. (신선도 점수 향상)"
      );
    }
  }
  ...
  if (smartBlocks.length < 5) {
    insights.push(
      "연관 키워드가 적습니다. 더 다양한 키워드로 콘텐츠를 확장해보세요. (노출 기회 증가)"
    );
  } else {
    insights.push(
      "연관 키워드가 풍부합니다. 이 키워드들을 활용하여 콘텐츠를 확장해보세요."
    );
  }
  ...
  if (popularPosts.length > 10) {
    insights.push(
      "경쟁이 치열합니다. 차별화된 콘텐츠로 차별점을 만들어보세요. (노출 점수 향상)"
    );
  }
  ...
  if (alsoSearched.length > 0) {
    insights.push(
      "함께 많이 찾는 검색어를 활용하여 콘텐츠를 확장해보세요. (노출 기회 증가)"
    );
  }
...
function generateMarketTrend(
  ...
  const totalData =
    popularPosts.length + smartBlocks.length + alsoSearched.length;
...
function calculateExposureProbability(
  cRankScore: number,
  totalResults: number
): number {
